...
X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size = _, random_state = 42) #The value of test_size (where the underscore is) must be between 0 and 1 (representing percentages). 

#Create a pipeline.
pipeline = Pipeline([("scaler", StandardScaler()), ("model", Ridge(alpha = _))]) #Replace the underscore with the value of the regularization term (alpha).
